...
from yolov3CPU.Detector import Object_Detector
...
dict_class={"person":1,"car":2,"motorbike":3,"bicycle":4,"truck":5,"bus":6,"backpack":7,"aeroplane":8,"stop sign":9,"horse":10,"boat":11,"traffic light":12,"train":13,"pottedplant":14,"bench":15}
...
def get_detections_1(PATH,img_files):
  detection_boundingbox=[]
  class_names=[]
  OB=Object_Detector()
  for i in range(len(img_files)):
    frame=cv2.imread(PATH+img_files[i])
    [detected_box, PredClass] = OB.localization(frame)
    frame_bboxes=[]
    frame_classes=[]
    for i in range(len(detected_box)):
      c=detected_box[i][2]+detected_box[i][0]
      d=detected_box[i][3]+detected_box[i][1]
      a=detected_box[i][0]
      b=detected_box[i][1]
      frame_bboxes.append([a,b,c,d])
      frame_classes.append(PredClass[i])
    detection_boundingbox.append(frame_bboxes)
    class_names.append(frame_classes)
  return detection_boundingbox,class_names
def get_detections_frame(img):
    frame=cv2.imread(img)
    OB=Object_Detector()
    [detected_box, PredClass] = OB.localization1(frame)
    frame_bboxes=[]
    frame_classes=[]
    for i in range(len(detected_box)):
      c=detected_box[i][2]+detected_box[i][0]
      ...
      frame_classes.append(PredClass[i])
    print(frame_bboxes)
    return (frame_bboxes,frame_classes)
...
if __name__ == "__main__":
    ...
    print(img_file)
    trackers = Re3Tracker()
    # detection_boundingbox,predicted_class=get_detections_1(PATH,img_file)
    counter=1
    ...
    print("total_frames-"+str(len(img_file)))
    for i in range(len(img_file)):
        ...
        t0=time.time()
        # print("DET_BBBOX", i)
        # frame_detections=get_detections_frame(img_path)
        # det_bbox=frame_detections[0]
        # print("frame-"+str(i))
        # pred_class=frame_detections[1]
        output=localize(img_path)
        det_bbox=output[0][2]
        pred_class=output[0][0]
        # print(det_bbox)
        # print("class_es:",pred_class)
        ...
        for ll in range(len(draw_bbox)):
            bbox=draw_bbox[ll]
            label=pred_labels[ll]
            x1=int(bbox[0])
            y1=int(bbox[1])
            x2=int(bbox[2])
            y2=int(bbox[3])
        ...
        # out.write(image)
        cv2.imwrite("/content/drive/My Drive/Re3/output2/"+str(counter)+'.jpg',image)
        counter+=1